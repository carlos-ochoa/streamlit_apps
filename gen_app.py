...
USE_ELITISM = st.sidebar.checkbox('Usar elitismo')
ELITISM = st.sidebar.number_input('¿Cuántas soluciones mantener en elitismo?', min_value = 0, max_value = 15, value = 0)
CROSSOVER = st.sidebar.slider('Porcentaje de cruza', min_value = 0.8, max_value = 1.0)
MUTATION = st.sidebar.slider('Porcentaje de mutación', min_value = 0.0, max_value = 0.2, value = 0.2)
N = st.sidebar.number_input('Cantidad de individuos', min_value = 0, max_value = 500, value = 100)
GENERATIONS = st.sidebar.number_input('Generaciones', min_value = 1, max_value = 200, value = 100)
ROUNDS = st.sidebar.slider('Rondas por torneo', min_value = 5, max_value = 10)
...
st.title('Problema del viajero resuelto con Algoritmos Genéticos')
st.write('El problema del viajero es un famoso problema de búsqueda, que dependiendo cómo se quiera resolver tiende a tener una complejidad mayor.')
st.write('En este caso intento resolverlo mediante una búsqueda heurística a través de algoritmos genéticos, que no me garantiza la convergencia a la mejor solución \
 pero que retornará una solución aceptable.')
expander = st.beta_expander('Narrativa del problema')
expander.write('El club de IA de la ESCOM desea planear un viaje por las 20 ciudades más pobladas de México para celebrar su inauguración, sin embargo quieren recorrer la menor distancia posible para no tener que cargar tanta gasolina. \
Pedro sugiere que encuentren una buena ruta usando un algoritmo genético, por lo que todos se ponen manos a la obra. \
Diseña un algoritmo genético con esta variación del problema del viajero')

expander_parametros = st.beta_expander('Parámetros del algoritmo')
expander_parametros.write('** _Usar elitismo_ ** : Indica al algoritmo si usar o no elitismo al pasar a siguientes generaciones')
expander_parametros.write('** _Soluciones mantenidas en elitismo_ ** : Permite indicar cuántos individuos conservar entre generaciones')
expander_parametros.write('** _Porcentaje de cruza_ ** : Indica la probabilidad de que dos individuos se crucen')
expander_parametros.write('** _Porcentaje de mutación_ ** : Indica la probabilidad de que un individuo mute')
expander_parametros.write('** _Cantidad de individuos_ ** : Indica cuántos individuos deseamos en nuestra población')
expander_parametros.write('** _Generaciones_ ** : Indica la cantidad de generaciones (1-200) que queremos que nuestro algoritmo corra')
expander_parametros.write('** _Rondas de torneo_ ** : Permite seleccionar cuántas rondas deben hacerse en la selección por torneo')
...
if CROSSOVER + MUTATION > 1:
    st.warning('Los porcentajes de mutacion y cruza deben sumar 1')
    valores_validos = False
else:
    valores_validos = True

if iniciar and valores_validos:
    for gen in range(GENERATIONS):
        ...
        gen_perc = gen / GENERATIONS
        progreso.progress(gen_perc)

if len(gens) == GENERATIONS:
    st.write('Costo por generaciones')
    st.line_chart(best)
    st.balloons()
    st.write('La solución encontrada es: ' , better_solution.get_genotype())
    st.write('Con costo: ', better_solution.get_fitness())
#plt.plot(gens,best)
#plt.show()
